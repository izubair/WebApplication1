    ...
    return declare(null, {
        ...
        */
        _addToGraphicsLayer: function (evt) {
					...
					// IZ Edits
					//alert("Inside draw-complete!");
					console.log("Inside draw-complete!");					
						
					// First check if within CC Jurisdiction
					
					var lat = evt.geometry.getLatitude();
					var lon = evt.geometry.getLongitude();
					...
					GetStatePlaneCoord(lat, lon, coords);
					
					GetJurisdication(coords.X, coords.Y, jurisObj);
					
					var jurStr = jurisObj.juris; 
					
					//document.getElementById("jurisdLabel").innerHTML = jurisObj.juris;
					if(jurStr.indexOf("CC ") == -1){
						
						alert("The selected location is not within Clark County jurisdiction. It is in " + jurisObj.juris);
						return;
					}
